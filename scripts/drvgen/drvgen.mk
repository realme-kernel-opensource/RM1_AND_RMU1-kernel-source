...
#Bin.Li@BSP.Fingerprint.Basic, 2017/08/15, modify for img sign
    ifneq ($(filter full_oppo6771_17331 full_oppo6771_17197 full_oppo6771_17061 full_oppo6771_17067, $(TARGET_PRODUCT)),)
	$(srctree)/../build/tools/Sign_Client/signature_client_17331 --project=17331 --input_path=$(DRVGEN_OUT)/ --input_name=odmdtbo.img --output_path=$(DRVGEN_OUT)/signed/
		cp $(DRVGEN_OUT)/signed/odmdtbo.img $(DRVGEN_OUT)/odmdtbo.img
    endif
    ifneq ($(filter full_oppo6771_18011 full_oppo6771_18311 full_oppo6771_18611, $(TARGET_PRODUCT)),)
	$(srctree)/../build/tools/Sign_Client/signature_client_18011 --project=17331 --input_path=$(DRVGEN_OUT)/ --input_name=odmdtbo.img --output_path=$(DRVGEN_OUT)/signed/
		cp $(DRVGEN_OUT)/signed/odmdtbo.img $(DRVGEN_OUT)/odmdtbo.img
    endif
    ifeq ($(shell echo $(TARGET_PRODUCT) | sed -e 's/_1.*//g'), full_oppo6763)
	$(srctree)/../build/tools/Sign_Client/signature_client_17111 --project=17111 --input_path=$(DRVGEN_OUT)/ --input_name=odmdtbo.img --output_path=$(DRVGEN_OUT)/signed/
		cp $(DRVGEN_OUT)/signed/odmdtbo.img $(DRVGEN_OUT)/odmdtbo.img
    endif
#endif /*VENDOR_EDIT*/
.PHONY: odmdtboimage
