...
void checkSMILarb0(uint32_t addr, uint32_t index)
{
	uint32_t regValue;
	...
	if (regValue) {
		CMDQ_LOG(
			"[MDP][Abnormal] Job Unfinish, larb 0:(0x%x, %d, 0x%x)\n",
			addr, index, regValue);
	} else
		return;
