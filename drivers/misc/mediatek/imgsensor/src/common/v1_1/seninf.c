...
int seninf_set_mclk_for_detect(MUINT8 pcEn, MUINT32 freq)
{
	int ret = 0;
	int i = 0;
	struct SENINF *pseninf = &gseninf;
	ACDK_SENSOR_MCLK_STRUCT pmclk;
	memset(&pmclk, 0, sizeof(ACDK_SENSOR_MCLK_STRUCT));
	...
	PK_DBG("[SENSOR_DETECT] set mclk for detect \n");
	for (i = SENINF_CLK_TG_0; i < SENINF_CLK_TG_MAX_NUM; i++) {
		pmclk.TG = i;
		ret = seninf_clk_set(&pseninf->clk, &pmclk);
	}
	if (pcEn == OFF) {
		seninf_clk_release(&pseninf->clk);
	}
