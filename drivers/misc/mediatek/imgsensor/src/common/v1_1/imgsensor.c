...
#ifdef VENDOR_EDIT
/*Caohua.Lin@Camera.Driver 20180210 add for sensor id detect*/
#include<linux/proc_fs.h>
#include "seninf.h"
static MINT32 gCurCameraId = 0;
static MINT32 gDetectSensorId = 0;
#endif
...
int imgsensor_set_driver(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (is_project(OPPO_17197)) {
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
				imgsensor_custom_config_for_17197[psensor_inst->sensor_idx].i2c_dev);
 	/*Caohua.Lin@Camera.Driver  add for 17175  board 20180205*/
		...
		switch (get_Operator_Version()) {
			...
			{
				PK_INFO("This project is 17171 and Operator_Version = %d\n", get_Operator_Version());
				imgsensor_i2c_init(&psensor_inst->i2c_cfg,
						imgsensor_custom_config_for_17171[psensor_inst->sensor_idx].i2c_dev);
				break;
			}

			default:
			{
				PK_INFO("This project is 17175 and Operator_Version = %d\n", get_Operator_Version());
				imgsensor_i2c_init(&psensor_inst->i2c_cfg,
						imgsensor_custom_config_for_17175[psensor_inst->sensor_idx].i2c_dev);
				break;
			}
		}
		...
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
						imgsensor_custom_config_for_18311[psensor_inst->sensor_idx].i2c_dev);
	} else {
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	snprintf(mtk_ccm_name,
		 sizeof(mtk_ccm_name),
		 "%s\nSecureType(0:NORMAL, 1:STATIC,2:DYNAMIC)=%2d",
		 mtk_ccm_name, info.sensorSecureType);
...
#ifdef CONFIG_MTK_CAM_SECURITY_SUPPORT
static void imgsensor_secure_close(void)
{
	int i = 0;
	UINT32 isSecure = 0;
	UINT32 isSecureSize = sizeof(isSecureSize);
	int ret = 0;
	struct IMGSENSOR *pimgsensor = &gimgsensor;
	struct IMGSENSOR_SENSOR_INST *psensor_inst = NULL;
	SENSOR_FUNCTION_STRUCT	     *psensor_func = NULL;

	for (i = 0; i < IMGSENSOR_SENSOR_IDX_MAX_NUM; i++) {
		psensor_inst = &pimgsensor->sensor[i].inst;
		psensor_func = pimgsensor->sensor[i].pfunc;
		if (psensor_func && psensor_func->SensorFeatureControl && psensor_inst) {
			imgsensor_mutex_lock(psensor_inst);
			psensor_func->psensor_inst = psensor_inst;

			ret = psensor_func->SensorFeatureControl(
			SENSOR_FEATURE_SET_AS_SECURE_DRIVER, (UINT8 *)&isSecure, &isSecureSize);
			PK_DBG("SECURE SENSOR RELEASE!\n  %d", ret);
			imgsensor_mutex_unlock(psensor_inst);
		}
	}
}
...
#ifdef CONFIG_MTK_CAM_SECURITY_SUPPORT
		imgsensor_secure_close();/* to reset sensor status */
...
#ifdef VENDOR_EDIT
/*Caohua.Lin@Camera.Driver 20180210 add for sensor id detect*/
static ssize_t SENSOR_HW_WRITE( struct file *file, const char __user *buffer, size_t count,
                                                                     loff_t *data)
{
	char regBuf[64] = {'\0'};
	...
	gCurCameraId = regBuf[0] - '0';
	PK_DBG("[SENSOR_DETECT] current camera id is %d\n", gCurCameraId);

	return count;
}

static ssize_t SENSOR_HW_READ(struct file *filp, char __user *buff,
                size_t len, loff_t *data)
{
	...
	struct IMGSENSOR_SENSOR *psensor =
        imgsensor_sensor_get_inst(gCurCameraId);

	struct IMGSENSOR_SENSOR_INST *psensor_inst = &psensor->inst;

	ret = seninf_set_mclk_for_detect(ON, MCLK);
	if (ret == 0) {
		PK_DBG("[SENSOR_DETECT]gCurCameraId=%d,set mclk sucess!!!!\n", gCurCameraId);
	}

	if (!imgsensor_check_is_alive(psensor)) {
		PK_DBG("[SENSOR_DETECT] :[%d][%s]\n",
		    psensor_inst->sensor_idx,
		    psensor_inst->psensor_list->name);

		seninf_set_mclk_for_detect(OFF, MCLK);
		snprintf(value, sizeof(value), "%x", gDetectSensorId);
		PK_DBG("[SENSOR_DETECT]gCurCameraId=%d,value=%s\n", gCurCameraId, value);
		return simple_read_from_buffer(buff, len, data, value, 7);
	}

	PK_PR_ERR("[SENSOR_DETECT]Get %d sensor fail\n", gCurCameraId);
	seninf_set_mclk_for_detect(OFF, MCLK);
	snprintf(value, sizeof(value), "%x", 0xFFFF);
	return simple_read_from_buffer(buff, len, data, value, 7);
}
...
static const struct file_operations sensor_proc_fops = {
	.owner = THIS_MODULE,
	.read = SENSOR_HW_READ,
	.write = SENSOR_HW_WRITE,
};

static int sensor_proc_init(void)
{
	int ret = 0;
	struct proc_dir_entry *proc_entry = proc_create_data("oppo_cam_sensor", 0666, NULL, &sensor_proc_fops, NULL);
	if (proc_entry == NULL)
	{
		ret = -1;
		PK_DBG("[SENSOR_DETECT]Error! Couldn't create sensor proc entry\n");
	}
	return ret;
}
#endif /*VENDOR_EDIT*/
