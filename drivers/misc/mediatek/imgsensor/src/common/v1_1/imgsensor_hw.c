...
enum IMGSENSOR_RETURN imgsensor_hw_init(struct IMGSENSOR_HW *phw)
{
	...
	for (i = 0; i < IMGSENSOR_SENSOR_IDX_MAX_NUM; i++) {
		...
		if (is_project(OPPO_17197)) {
			pcust_pwr_cfg = imgsensor_custom_config_for_17197;
		/*Caohua.Lin@Camera.Driver  add for 17175  board 20180205*/
			...
			switch (get_Operator_Version()) {
				...
				{
					pcust_pwr_cfg = imgsensor_custom_config_for_17171;
				...
				default:
				{
					pcust_pwr_cfg = imgsensor_custom_config_for_17175;
					PK_DBG("17175's pwr_cfg and Operator_Version = %d\n", get_Operator_Version());
					break;
				}
			}
		} else if (is_project(OPPO_18311) || is_project(OPPO_18011) || is_project(OPPO_18611)) {
			PK_DBG("18311/18011's pwr_cfg and Operator_Version = %d\n", get_Operator_Version());
			pcust_pwr_cfg = imgsensor_custom_config_for_18311;
		} else {
		...
		enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	if (strlen(pcustomize_sensor) > 3 && !strstr(pcustomize_sensor, curr_sensor_name)) {
		PK_DBG("IMGSENSOR_RETURN_ERROR \n");
		return IMGSENSOR_RETURN_ERROR;
	}
#endif
