		...
		if (regVCAMAF == NULL) {
			...
			if (node) {
				...
				if (strncmp(CONFIG_ARCH_MTK_PROJECT, "k71v1_64_bsp_fhdp", 17) == 0)
					...
					#ifndef VENDOR_EDIT
					/*Feng.Hu@Camera.Driver 20171201 add for af power up*/
...
static long AF_SetMotorName(__user struct stAF_MotorName *pstMotorName)
{
	...
	#ifdef VENDOR_EDIT
	/*Feng.Hu@Camera.Driver 20171222 add for evb driverIC*/
	if (is_project(OPPO_17331) && (get_PCB_Version() == HW_VERSION__10)) {
		memset(&stMotorName.uMotorName, 0, sizeof(stMotorName.uMotorName));
		memcpy(&stMotorName.uMotorName, AFDRV_BU6429AF, sizeof(AFDRV_BU6429AF));
	}
	#endif
