...
/*liping-m@PSW.MM.Display.LCD.Stability, 2018/07/04, add for hx83112a lcd esd read reg*/
...
static int primary_display_check_recovery_worker_kthread(void *data)
{
	...
	#ifdef VENDOR_EDIT
	/*
	* Ling.Guo@PSW.MM.Display.LCD.Machine, 2018/01/25,
		...
		*/
		...
		if (esd_check_enable) {
			...
			do {
				...
				#ifdef VENDOR_EDIT
				/*
				* Ling.Guo@PSW.MM.Display.LCD.Stability, 2018/01/12,
				* add for esd recovery
				*/
				msleep(2000);
				#endif /*VENDOR_EDIT*/
...
int primary_display_esd_recovery(void)
{
	...
	*/
	...
	/*Shizeke@PSW.MM.Display.LCD.Stability, 2018/08/15, add for realme 18611 to set backlight after esd recovery*/
	if (is_project(17197) || is_project(17199)) {
		disp_lcm_set_backlight(primary_get_lcm(),NULL,esd_recovery_backlight_level);
	} else if (is_project(17331) || is_project(17061) || is_project(17175) || is_project(18311) || is_project(18011) || is_project(18611)) {
...
static int external_display_check_recovery_worker_kthread(void *data)
{
	...
	while (1) {
		#ifndef VENDOR_EDIT
		/*
		* LiPing-M@PSW.MM.Display.LCD.Stability.1111262, 2017/12/13,
