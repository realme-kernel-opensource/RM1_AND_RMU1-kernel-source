...
int mipi_clk_change(int msg, int en)
{
	...
	*/
	...
	if (en) {
		...
		/*shizeke@PSW.MM.Display.LCD.Stability, 2018/08/15, add for realme 18611 mipi clk change 514.5->529m*/
		if (get_Operator_Version() == 25 ) {
		    def_data_rate = 1058;
		    def_dsi_hbp = 0xC0; /* adaptive HBP value */
		} else if (is_project(18311) || is_project(18011) || is_project(18611)) {
		    def_data_rate = 1058;
	...
	#ifdef VENDOR_EDIT
	/*Guoqiang.jiang@PSW.MM.Display.LCD.Stability, 2018/05/11, add for mipi clk change*/
	/*Shizeke@PSW.MM.Display.LCD.Stability, 2018/8/15, add for mipi clk change in realme 18611*/
	if (_is_power_on_status(DISP_MODULE_DSI0)) {
		...
		if (get_Operator_Version() == 25 || is_project(18311) || is_project(18011) || is_project(18611)) {
			if ((DSI_REG[0]->DSI_MODE_CTRL.MODE) != 0) {
