...
#ifndef VENDOR_EDIT
//Fuchun.Liao@BSP.CHG.Basic 2017/11/23 modify for power issue
				wake_unlock(&musb->usb_wakelock);
#else
				pr_err("%s skip unlock usb_wakelock\n", __func__);
#endif /* VENDOR_EDIT */
			}
#ifdef VCORE_OPS_DEV
			vcore_op(0);
...
#else
				pr_err("%s skip require usb_wakelock\n", __func__);
#endif /* VENDOR_EDIT */
			}
...
//Fuchun.Liao@BSP.CHG.Basic 2017/11/23 modify for power issue
				wake_unlock(&musb->usb_wakelock);
#else
				pr_err("%s skip release usb_wakelock\n", __func__);
#endif /* VENDOR_EDIT */
			}
#ifdef VCORE_OPS_DEV
			vcore_op(0);
...
#endif
#ifndef VENDOR_EDIT
/* Qiao.Hu@BSP.BaseDrv.CHG.Basic, 2017/11/24, delete for chargertype modefy */
...
bool usb_cable_connected(void)
{
	...
	if (mu3d_force_on) {
		...
		#ifdef VENDOR_EDIT
		/* Qiao.Hu@BSP.BaseDrv.CHG.Basic, 2017/11/24, delete for chargertype modefy */
		chg_type = _mu3d_musb->charger_mode = g_chr_type;
		#else
