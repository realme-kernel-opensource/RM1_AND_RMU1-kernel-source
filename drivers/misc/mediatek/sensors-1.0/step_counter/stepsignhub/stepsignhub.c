...
#ifdef CONFIG_PM
static int stepsignhub_pm_event(struct notifier_block *notifier, unsigned long pm_event,
			void *unused)
{
	struct step_chub_ipi_data *obj = &obj_ipi_data;
...
static struct notifier_block stepsignhub_pm_notifier_func = {
	.notifier_call = stepsignhub_pm_event,
	.priority = 0,
};
#endif /* CONFIG_PM */
...
#ifdef CONFIG_PM
	if (register_pm_notifier(&stepsignhub_pm_notifier_func)) {
		STEP_CDS_PR_ERR("Failed to register PM notifier.\n");
		goto exit;
	}
#endif /* CONFIG_PM */
	return 0;
