	...
	unsigned char *data, unsigned int size)
{
	...
	if (!xtalk_readed) {
		...
		if (Sensor_ID == EEPROM_SENSORID_IMX371) {
			iReadData(XTALK_DATA_FLAG, 1, &xtalk_flag);
			if (xtalk_flag == 1) {
				pr_debug("imx371's SensorID = 0x%x to read 4cell algorithm\n", Sensor_ID);
				iReadData(XTALK_START_ADDR, XTALK_DATA_SIZE, data_xtalk);
				xtalk_readed = 1;
			} else {
				pr_debug("Warning,imx371'otp not have cross-talk calibration data !\n");
			}
		}
		...
		if (Sensor_ID == EEPROM_SENSORID_S5K3P9SP) {
			...
			if (xtalk_flag == 1) {
				pr_debug("s5k3p9sp's Sensor_ID = 0x%x to read 4cell algorithm\n", Sensor_ID);
...
#ifdef VENDOR_EDIT
/*Caohua.Lin@Camera.Driver 20180702 add for imx371 crosstalk*/
unsigned int brcb032gwz_read_4cell_from_eeprom(char *data)
{
	if (data != NULL) {
...
/*Caohua.Lin@Camera.Driver 20180707 add for s5k3p9sp crosstalk*/
unsigned int brcb032gwz_read_4cell_from_eeprom_s5k3p9sp(char *data)
{
	if (data != NULL) {
