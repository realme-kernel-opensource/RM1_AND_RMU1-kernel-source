...
#endif
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (ptempbuf->sensorID == S5K3P9SP_SENSOR_ID && ptempbuf->deviceID == SUB_CAMEAR) {
			PK_DBG("modify for reading s5k3p9sp's OTP because of s5k3p9sx\n");
			ptempbuf->sensorID = S5K3P9SP_F_SENSOR_ID;
		}
