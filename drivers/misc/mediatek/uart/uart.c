...
#ifdef VENDOR_EDIT
//Qiao.Hu@BSP.CHG.Basic, 2017/12/18, add for switch uart mode for chargerid.
...
#ifdef VENDOR_EDIT
//Qiao.Hu@BSP.CHG.Basic, 2017/12/18, add for switch uart mode for chargerid.
bool oppo_get_uart_status(void)
{
	...
	#else
		pr_err("%s: NO release_flag\n",__func__);
		return true;
	#endif
}
static struct uart_driver mtk_uart_drv_no_console = {
	.owner = THIS_MODULE,
	.driver_name = DRV_NAME,
	.dev_name = "ttyMT",
	.major = UART_MAJOR,
...
#ifdef VENDOR_EDIT
//#Qiao.Hu@BSP.CHG.Basic, 2017/12/18, add for switch uart mode for chargerid.
	if(oppo_get_uart_status() == false){
		serial_pinctrl = devm_pinctrl_get(&pdev->dev);
		if (IS_ERR_OR_NULL(serial_pinctrl)) {
			pr_err("%s ppp get serial pinctrl null\n", __func__);
		} else {
			pr_err("%s ppp get serial pinctrl success\n", __func__);
			rx_pinctrl_state_diable = pinctrl_lookup_state(serial_pinctrl, "uart0_rx_gpio");
			if (IS_ERR_OR_NULL(rx_pinctrl_state_diable)) {
				pr_err("%s ppp get serial_pinctrl_state fail\n", __func__);
			} else {
				pr_err("%s ppp get serial_pinctrl_state success\n", __func__);
		...
		pr_err("%s: ppp boot with uartlog false\n",__func__);
		return -ENODEV;
	}
...
#else
	if(oppo_get_uart_status() == true){
		err = uart_add_one_port(&mtk_uart_drv, &uart->port);
	} else {
		err = uart_add_one_port(&mtk_uart_drv_no_console, &uart->port);
	}
...
//Qiao.Hu@BSP.CHG.Basic, 2017/12/18, add for switch uart mode for chargerid.
	...
	if(oppo_get_uart_status() == true){
		...
		err = uart_remove_one_port(&mtk_uart_drv_no_console, &uart->port);
	}
#endif /*VENDOR_EDIT*/
...
//Qiao.Hu@BSP.CHG.Basic, 2017/12/18, add for switch uart mode for chargerid.
		...
		if(oppo_get_uart_status() == true){
			...
			ret = uart_suspend_port(&mtk_uart_drv_no_console, &uart->port);
		}
#endif /*VENDOR_EDIT*/
...
//Qiao.Hu@BSP.CHG.Basic, 2017/12/18, add for switch uart mode for chargerid.
		...
		if(oppo_get_uart_status() == true){
			...
			ret = uart_resume_port(&mtk_uart_drv_no_console, &uart->port);
		}
#endif /*VENDOR_EDIT*/
		spin_unlock_irqrestore(&mtk_uart_bt_lock, flags);
...
//Qiao.Hu@BSP.CHG.Basic, 2017/12/18, add for switch uart mode for chargerid.
	...
	if(oppo_get_uart_status() == true){
		...
		ret = uart_register_driver(&mtk_uart_drv_no_console);
	}
#endif /*VENDOR_EDIT*/
...
//Qiao.Hu@BSP.CHG.Basic, 2017/12/18, add for switch uart mode for chargerid.
		...
		if(oppo_get_uart_status() == true){
			...
			uart_unregister_driver(&mtk_uart_drv_no_console);
		}
#endif /*VENDOR_EDIT*/
		return ret;
	}
