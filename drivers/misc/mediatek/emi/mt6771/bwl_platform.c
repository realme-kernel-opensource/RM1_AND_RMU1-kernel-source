...
#define CEN_SNST		(CEN_EMI_BASE + 0x7F8)
#define CHN_SNST		(CHN_EMI_BASE + 0x9F8)
...
void resume_decs(void __iomem *CEN_EMI_BASE)
{
	...
	value = readl(IOMEM(CEN_SNST)) & 0xF0FFFFFF;
	mt_reg_sync_writel(value | 0x81000000, CEN_SNST);

	for (i = 0; i < MAX_CH; i++) {
		CHN_EMI_BASE = mt_chn_emi_base_get(i);
		value = readl(IOMEM(CHN_SNST));
		mt_reg_sync_writel(value | 0x80000000, CHN_SNST);
	}
}
