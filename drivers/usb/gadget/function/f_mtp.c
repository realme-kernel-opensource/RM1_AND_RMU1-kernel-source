...
#endif
static bool read_wait_in;
static u64 read_wait_ns;
...
requeue_req:
	...
	//			dev->rx_done || dev->state != STATE_BUSY);
	{
		...
		read_wait_ns += (timeval_to_ns(&tv_after) - timeval_to_ns(&tv_before));
		read_wait_in = false;
	}
	if (dev->state == STATE_CANCELED) {
		r = -ECANCELED;
...
static bool write_wait_in;
static u64 write_wait_ns;
		...
		//		|| dev->state != STATE_BUSY));
		{
			...
			ret = wait_event_interruptible(dev->write_wq,
					((req = mtp_req_get(dev, &dev->tx_idle))
					 || dev->state != STATE_BUSY));
			do_gettimeofday(&tv_after);
			write_wait_ns += (timeval_to_ns(&tv_after) - timeval_to_ns(&tv_before));
			write_wait_in = false;
		}
		if (!req) {
			r = ret;
...
void trigger_rx_concurrent(void)
{
	...
	for (i = 0; i < RX_REQ_MAX; i++) {
        ...
        //#ifndef VENDOR_EDIT
        //Xuefeng.Peng@PSW.AD.Usb.Mtp.1203013, 2018/01/10, Modify for mtp file size large than 2G will exceed
		//MTP_RX_CONCURRENT_DBG("count<%d>, usb_read_count<%d>\n", (int)count, (int)usb_read_count);
        //#else
        MTP_RX_CONCURRENT_DBG("count<%d>, usb_read_count<%ld>\n", (int)count, usb_read_count);
        //#endif/*VENDOR_EDIT*/
		if (!count)
			break;
	}
...
static bool file_wait_in;
static u64 file_wait_ns;
static long mtp_ioctl(struct file *fp, unsigned code, unsigned long value)
{
	...
	case MTP_SEND_FILE_WITH_HEADER:
	{
		 ...
		 */
		{
			...
			file_wait_in = true;
			MYDBG("%s operation, len<%d>\n", code == MTP_RECEIVE_FILE ? "recv" : "send",
					(int)dev->xfer_file_length);
			do_gettimeofday(&tv_before);
			...
			do_gettimeofday(&tv_after);
			file_wait_ns += (timeval_to_ns(&tv_after) - timeval_to_ns(&tv_before));
			file_wait_in = false;
				//queue_work(dev->wq, work);
				/* wait for operation to complete */
				//flush_workqueue(dev->wq);
...
static bool read_in;
static bool write_in;
static bool ioctl_in;
static u64 read_ns;
static u64 write_ns;
static u64 ioctl_ns;
static ssize_t my_mtp_read(struct file *fp, char __user *buf,
	size_t count, loff_t *pos)
{
	ssize_t ret;
	struct timeval tv_before, tv_after;

	{
		...
		if (!inited) {
			...
			INIT_DEFERRABLE_WORK(&monitor_work, do_monitor_work);
			inited = true;
	...
	read_in = true;
	do_gettimeofday(&tv_before);
	ret = mtp_read(fp, buf, count, pos);
	do_gettimeofday(&tv_after);
	read_ns += (timeval_to_ns(&tv_after) - timeval_to_ns(&tv_before));
	read_in = false;
	return ret;
}
static ssize_t my_mtp_write(struct file *fp, const char __user *buf,
	size_t count, loff_t *pos)
{
	ssize_t ret;
	struct timeval tv_before, tv_after;
	...
	write_in = true;
	do_gettimeofday(&tv_before);
	ret = mtp_write(fp, buf, count, pos);
	do_gettimeofday(&tv_after);
	write_ns += (timeval_to_ns(&tv_after) - timeval_to_ns(&tv_before));
	write_in = false;
	return ret;
}
static long my_mtp_ioctl(struct file *fp, unsigned code, unsigned long value)
{
	long ret;
	struct timeval tv_before, tv_after;
	...
	do_gettimeofday(&tv_after);
	ioctl_ns += (timeval_to_ns(&tv_after) - timeval_to_ns(&tv_before));
	ioctl_in = false;
	return ret;
}
...
static void do_monitor_work(struct work_struct *work)
{
	MYDBG("read<%d,%lu>, read_wait<%d,%lu>\n",
			read_in, (unsigned long)read_ns,
			read_wait_in, (unsigned long)read_wait_ns);
	MYDBG("write<%d,%lu>, write_wait<%d,%lu>\n",
			write_in, (unsigned long)write_ns,
			write_wait_in, (unsigned long)write_wait_ns);
	MYDBG("ioctl<%d,%lu>, file<%d,%lu>\n",
			ioctl_in, (unsigned long)ioctl_ns,
			file_wait_in, (unsigned long)file_wait_ns);
