 ...
 **  Ran.Chen                    2018/06/06                  add fpc1023+2060 (vdd_io 3v)
 **  Hongdao.yu                  2018/08/10                  modify lock by kernel4.9
 **  Hongdao.yu                  2018/08/11                  add fpc1023+2060 reset for sdm450
 **  Shupeng.Zhou                2018/09/11                  add fpc1151 for sdm450
 ************************************************************************************/
...
#ifdef VENDOR_EDIT
/*zhoushupeng, 2018/08/10, use mutex instead of spin_lock to avoid sleep check of kernel 4.9, which would induce dump*/
#if CONFIG_OPPO_FINGERPRINT_PROJCT == 18171 || CONFIG_OPPO_FINGERPRINT_PROJCT == 18571 || CONFIG_OPPO_FINGERPRINT_PROJCT == 18573
    static DEFINE_MUTEX(fpc1020_mutex);
#else
/* ziqing.guo@BasicDrv.Sensor, 2016/01/26, modify for enable/disable irq */
...
#if CONFIG_OPPO_FINGERPRINT_PROJCT == 18171 || CONFIG_OPPO_FINGERPRINT_PROJCT == 18571 || CONFIG_OPPO_FINGERPRINT_PROJCT == 18573
        /*get ldo resource*/
        ldo_gpio = of_get_named_gpio(fpc1020->dev->of_node, "fpc,vdd_en", 0);
        if (ldo_gpio < 0) {
            dev_err(dev, "failed to get '%s'\n", "fpc,vdd_en");
            return ldo_gpio;
        }

        rc = devm_gpio_request(fpc1020->dev, ldo_gpio, "fpc,vdd_en");
        if (rc) {
            dev_err(dev, "Failed to request LDO GPIO %d\n", rc);
            return rc;
        }
