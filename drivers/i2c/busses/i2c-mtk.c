...
#ifdef VENDOR_EDIT
//tongfeng.huang@BSP.CHG.Basic, 2018/02/27 Add for i2c-6 reset, patch:REQ30000477623
...
static void dump_gpio_state(struct mt_i2c *i2c)
{
	void __iomem *gpio_base;
	void __iomem *pinctrl_rm_base;
	struct device_node *gpio_node;
	struct device_node *pinctrl_rm_node;
	u32 gpio1, gpio2;

	gpio_node = of_find_compatible_node(NULL, NULL, "mediatek,gpio");
	gpio_base = of_iomap(gpio_node, 0);
	pinctrl_rm_node = of_find_compatible_node(NULL, NULL,
						  "mediatek,pctl-5-syscfg");
	pinctrl_rm_base = of_iomap(pinctrl_rm_node, 0);

	if (i2c->id == 2) {
		/* SCL: GPIO_BASE+0x03C0[31:28] */
		/* SDA: GPIO_BASE+0x03D0[3:0]   */
		gpio1 = readl(gpio_base + 0x3c0);
		gpio2 = readl(gpio_base + 0x3d0);
		dev_info(i2c->dev, "scl mode[31:28] %x sda mode[3:0] %x\n",
			 gpio1, gpio2);
		dev_info(i2c->dev, "internal pull setting %x %x\n",
			 readl(pinctrl_rm_base + 0xf0),
			 readl(pinctrl_rm_base + 0x60));
		if ((gpio1 & (0xF << 28)) != (1 << 28)) {
			/* DIR				VALUE */
			/* SCL2 GPIO_BASE+0x0030[7]	GPIO_BASE+0x0130[7] */
			pr_info("wrong scl2 state dir[7] %x out[7] %x\n",
				readl(gpio_base + 0x30),
				readl(gpio_base + 0x130));
		}
		if ((gpio2 & (0xF)) != 0x1) {
			/* DIR				VALUE */
			/* SDA2 GPIO_BASE+0x0030[8]	GPIO_BASE+0x0130[8] */
			pr_info("wrong sda2 state dir[8] %x out[8] %x\n",
				readl(gpio_base + 0x30),
				readl(gpio_base + 0x130));
		}
	} else if (i2c->id == 4) {
		/* SCL: GPIO_BASE+0x03D0[7:4]  */
		/* SDA: GPIO_BASE+0x03D0[11:8] */
		gpio1 = readl(gpio_base + 0x3d0);
		dev_info(i2c->dev, "scl mode[7:4] %x sda mode[11:8] %x\n",
			 gpio1, gpio1);
		dev_info(i2c->dev, "internal pull setting %x %x\n",
			 readl(pinctrl_rm_base + 0xf0),
			 readl(pinctrl_rm_base + 0x60));
		if ((gpio1 & (0xF << 4)) != (1 << 4)) {
			/* DIR				VALUE */
			/* SCL4 GPIO_BASE+0x0030[9]	GPIO_BASE+0x0130[9] */
			pr_info("wrong scl4 state dir[9] %x out[9] %x\n",
				readl(gpio_base + 0x30),
				readl(gpio_base + 0x130));
		}
		if ((gpio1 & (0xF << 8)) != (1 << 8)) {
			/* DIR				VALUE */
			/* SDA4 GPIO_BASE+0x0030[10]	GPIO_BASE+0x0130[10] */
			pr_info("wrong sda4 state dir[10] %x out[10] %x\n",
				readl(gpio_base + 0x30),
				readl(gpio_base + 0x130));
		}
	}
...
static int mt_i2c_do_transfer(struct mt_i2c *i2c)
{
	...
	if (tmo == 0) {
		...
		if ((i2c->id == 2) || (i2c->id == 4)) {
			dev_info(i2c->dev, "vcamio %d\n",
			pmic_get_register_value(PMIC_RG_LDO_VCAMIO_EN));
			dump_gpio_state(i2c);
		}
		i2c_dump_info(i2c);
		...
		mt_i2c_init_hw(i2c);
		bus_state = i2c_readw(i2c, OFFSET_DEBUGSTAT);
		pr_info("bus state after reset %x\n", bus_state);
		if (i2c->ch_offset)
			i2c_writew_shadow(I2C_RESUME_ARBIT, i2c, OFFSET_START);
#ifdef VENDOR_EDIT
//tongfeng.huang@BSP.CHG.Basic, 2018/02/27 Add for i2c-6 reset, patch:REQ30000477623
		if (!strcmp(dev_name(i2c->dev), chg_i2c)) {
...
#ifdef VENDOR_EDIT
//tongfeng.huang@BSP.CHG.Basic, 2018/02/27 Add for i2c-6 reset, patch:REQ30000477623
	i2c->pctrl = devm_pinctrl_get(&pdev->dev);	/*add for oppo*/
#endif /*VENDOR_EDIT*/
