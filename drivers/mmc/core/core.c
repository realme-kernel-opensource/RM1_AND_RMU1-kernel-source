...
#ifdef VENDOR_EDIT
/* Fuchun.Liao@BSP.CHG.Basic 2018/08/27 modify for micron mcp hang ALPS04054718 */
#define EMMC_VENDOR_SAMSUNG 	1
#define EMMC_VENDOR_HYNIX 		2
#define EMMC_VENDOR_MICRON		3
static unsigned int emmc_vendor = 0;
static int __init emmc_vendor_setup(char *str)
{
	 sscanf(str, "%d", &emmc_vendor);
	 pr_err("%s emmc_vendor:%d\n", __func__, emmc_vendor);
     return 1;
}
__setup("emmc_vendor=", emmc_vendor_setup);
#endif /* VENDOR_EDIT */

static void mmc_reset_cq(struct mmc_host *host)
{
	static int reset_fail_count = 0;
...
#ifdef VENDOR_EDIT
/* Fuchun.Liao@BSP.CHG.Basic 2018/08/27 modify for micron mcp hang ALPS04054718 */
		if (emmc_vendor == EMMC_VENDOR_MICRON) {
			reset_fail_count++;
			if (reset_fail_count > 10) {
				panic("micron mcp reinit fail\n");
			}
		}
#endif /* VENDOR_EDIT */
		...
		if (emmc_vendor == EMMC_VENDOR_MICRON) {
			if (resp == 0) {
				if (!not_ready_time)
					...
					/* no task ready for over 10 sec */
					pr_notice("mmc0: error: task not ready over 10s\n");
					msleep(2000);
