...
#ifdef VENDOR_EDIT
/* Yongzhi.Zhang@PSW.MM.AudioDriver.Machine.1223139, 2018/01/13,
...
{
#ifndef VENDOR_EDIT
/* Wensheng.Zha@PSW.Multimedia.AudioDriver.Codec, 2018/01/24, Modify for controlling speaker PA */
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	...
	if (enable) {
		if (is_project(OPPO_17061)) {
			AudDrv_GPIO_Speaker_Amp_Select(true, AW_MODE_3);
			msleep(2);
		}
	} else {
		if (is_project(OPPO_17061)) {
			AudDrv_GPIO_Speaker_Amp_Select(false, AW_MODE_3);
			msleep(5);
		}
	}
#endif /* VENDOR_EDIT */
	...
	if (enable) {
		if (GetAdcStatus() == false) {
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				/* phone mic */
#ifdef VENDOR_EDIT
				/* Yongzhi.Zhang@PSW.MM.AudioDriver.Codec, 2017/12/14,
				...
				/* headset mic */
#ifdef VENDOR_EDIT
				/* Yongzhi.Zhang@PSW.MM.AudioDriver.Codec, 2018/03/27,
		...
		if (GetAdcStatus() == false) {
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				...
				/* Disable MICBIAS1 */
#ifdef VENDOR_EDIT
				/* Yongzhi.Zhang@PSW.MM.AudioDriver.Codec, 2018/03/10,
