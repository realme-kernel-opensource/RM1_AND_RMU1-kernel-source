...
#ifdef VENDOR_EDIT
/* Wensheng.Zha@PSW.Multimedia.AudioDriver.Codec, 2018/01/26, Add for audio */

#include <linux/spinlock.h>
#include <linux/device.h>
#include <linux/of_gpio.h>

static DEFINE_SPINLOCK(gpio_spk_pa_lock);
static int gpio_spk_pa_en;
void AudDrv_GPIO_Speaker_Amp_Select(int bEnable, int mode)
{
	...
	pr_err("%s bEnable = %d\n", __func__, bEnable);
	if ((mode < AW_MODE_3) || (mode > AW_MODE_10)) {
		pr_err("%s mode error\n", __func__);
		return;
	}
	spin_lock_irqsave(&gpio_spk_pa_lock, flags);
	if (bEnable) {
		for (i = 0; i < mode; i++) {
			gpio_set_value(gpio_spk_pa_en, AW_OUTPUT_LOW);
			udelay(2);
			gpio_set_value(gpio_spk_pa_en, AW_OUTPUT_HIGH);
			udelay(2);
		}
	} else {
		gpio_set_value(gpio_spk_pa_en, AW_OUTPUT_LOW);
	...
	spin_unlock_irqrestore(&gpio_spk_pa_lock, flags);
	return;
}
...
#ifdef VENDOR_EDIT
	/* Wensheng.Zha@PSW.Multimedia.AudioDriver.Codec, 2018/01/26, Add for audio */
    gpio_spk_pa_en = of_get_named_gpio(node->of_node, "gpio_spk_pa_en", 0);
    if (gpio_spk_pa_en < 0) {
		pr_err("the gpio pin:gpio_spk_pa_en not found\n");
    } else {
		if (gpio_is_valid(gpio_spk_pa_en)) {
			rc = gpio_request(gpio_spk_pa_en, "spk_pa_en");
			if (rc) {
				pr_err("unable to request gpio_spk_pa_en %d\n", gpio_spk_pa_en);
			}
		}
    }
#endif /* VENDOR_EDIT */
